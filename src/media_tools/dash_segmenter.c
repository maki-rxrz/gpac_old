...
static GF_Err gf_media_isom_segment_file(GF_ISOFile *input, const char *output_file, Double max_duration_sec, GF_DASHSegmenterOptions *dash_cfg, GF_DashSegInput *dash_input, Bool first_in_set)
{
	...
	Bool split_seg_at_rap = dash_cfg ? dash_cfg->segments_start_with_rap : GF_FALSE;
	...
	Bool use_url_template = dash_cfg ? dash_cfg->use_url_template : GF_FALSE;
	...
	if (! dash_moov_setup) {
		e = gf_isom_clone_movie(input, output, GF_FALSE, GF_FALSE);
		if (e) goto err_exit;
...
restart_fragmentation_pass:
	...
	if (dash_cfg) switch_segment=GF_TRUE;
